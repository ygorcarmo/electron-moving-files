...
// add files to custom destination
exports.addFilesToDestination = (files = [], location) => {
  // ensure `appDir` exists
  const dest = "C:\\Users\\ygorc\\Downloads\\newfolder";
  // const dest = document.getElementById("loco").innerHTML;
  fs.ensureDirSync(location);

  // copy `files` recursively (ignore duplicate file names)
  files.forEach((file) => {
    const filePath = path.resolve(location, file.name);
    if (!fs.existsSync(filePath)) {
      fs.copyFileSync(file.path, filePath);
    }
  });
...
exports.deleteFile = (filename) => {
  ...
  notification.filesDeleted(filename);
};
...
exports.getdestination = (dest) => {
  const filePath = path.resolve(appDir, dest);
  return filePath;
};
